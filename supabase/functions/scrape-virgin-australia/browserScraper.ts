...
import { scrapeVirginAustraliaWithHTTP } from './httpScraper.ts';

export async function scrapeVirginAustraliaAwardFlights(params: FlightSearchParams): Promise<FlightResult[]> {
  console.log('Starting Virgin Australia award flight scraping...');
  
  // First try HTTP-based scraping (more reliable in Edge Function environment)
  try {
    console.log('Attempting HTTP-based scraping...');
    const httpData = await scrapeVirginAustraliaWithHTTP(params);
    
    if (httpData && httpData.length > 0) {
      console.log('HTTP scraping successful, parsing data...');
      const flights = parseFlightDataFromAPI(httpData, params);
      
      if (flights.length > 0) {
        console.log(`Successfully extracted ${flights.length} flights via HTTP`);
        return flights;
      }
    }
  } catch (error) {
  ...
  // Fallback to Playwright if HTTP fails (though this may not work in Edge Functions)
  console.log('Falling back to browser-based scraping...');
  return await tryPlaywrightScraping(params);
}

async function tryPlaywrightScraping(params: FlightSearchParams): Promise<FlightResult[]> {
  try {
